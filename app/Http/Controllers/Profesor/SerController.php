...
class SerController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            // Obtener las actividades filtradas por asignatura ->where('id_curso', $idParalelo) $idParalelo
            $criterios = Criterio::whereHas('notas', function ($q) use ($asignaturaId, $idTrimestre, $idParalelo) {
                $q->where('id_materia', $asignaturaId)->where('id_curso', $idParalelo)->where('id_trimestre', $idTrimestre)->where('id_dimencion', 1);
            })->get();
            ...
            $estudiantes = Estudiante::where('id_curso', $idParalelo)
                ->with(['notas' => function ($query) use ($asignaturaId, $idTrimestre) {
                    $query->where('id_materia', $asignaturaId)
                        ...
                        ->where('id_dimencion', 1) //dimension del SER
            ...
            return view('profesor.ser.index', compact(
    ...
    {
        //dd($request);
        // Valor por defecto: primera materia y primer trimestre
