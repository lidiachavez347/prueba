...
class SaberController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            // Obtener las actividades filtradas por asignatura ->where('id_curso', $idParalelo) ->where('id_curso', $idParalelo) $idParalelo
            $criterios = Criterio::whereHas('notas', function ($q) use ($asignaturaId, $idTrimestre, $idParalelo) {
                $q->where('id_materia', $asignaturaId)->where('id_curso', $idParalelo)->where('id_trimestre', $idTrimestre)->where('id_dimencion', 2);
            })->get();
            ...
            $estudiantes = Estudiante::where('id_curso', $idParalelo)
                ->with(['notas' => function ($query) use ($asignaturaId, $idTrimestre) {
                    $query->where('id_materia', $asignaturaId)
                        ...
                        ->where('id_dimencion', 2) //dimension del SABER
            ...
            return view('profesor.saber.index', compact(
    ...
    public function update(Request $request, $id)
    {
        ...
        // 1 Actualizar nombre del criterio
        $criterio->descripcion = $request->descripcion;
        $criterio->save();

        // 2Actualizar notas de todos los estudiantes
