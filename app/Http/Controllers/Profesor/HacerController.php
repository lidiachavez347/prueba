...
class HacerController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $criterios = Criterio::whereHas('notas', function ($q) use ($asignaturaId, $idTrimestre, $idParalelo) {
                $q->where('id_materia', $asignaturaId)->where('id_curso', $idParalelo)->where('id_trimestre', $idTrimestre)->where('id_dimencion', 3);
            })->get();
            ...
            $estudiantes = Estudiante::where('id_curso', $idParalelo)
                ->with(['notas' => function ($query) use ($asignaturaId, $idTrimestre) {
                    $query->where('id_materia', $asignaturaId)
                        ...
                        ->where('id_dimencion', 3) //dimension del SABER
            ...
            return view('profesor.hacer.index', compact(
