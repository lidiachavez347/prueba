...
<div class="modal-footer">
    <a href="{{ route('admin.roles.create') }}" class="btn btn-dark" data-toggle="tooltip" data-placement="left" title="Adicionar">
            ...
            <tr id="permiso-{{ $role->id }}">
                ...
                <td class="text-start">
                            @foreach($role->permissions as $perm)
                                <span class="badge bg-info text-dark mb-1">{{ $perm->description }}</span>
                            @endforeach
                        </td>

                <td>{{ \Carbon\Carbon::parse($role->created_at)->locale('es')->isoFormat(' D \d\e MMMM \d\e\l Y') }} </td>
                ...
                <td>
                    ...
                    <a href="javascript:void(0);" onclick="abrirModalEditar('{{$role->id}}')" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Editar">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Botón Eliminar Permiso -->
                    <button onclick="eliminarPermiso('{{ $role->id }}')" class="btn btn-danger btn-sm" aria-label="eliminar" data-toggle="tooltip" data-placement="top" title="Eliminar">
                    ...
                    <div class="modal fade" id="editarUsuarioModal" tabindex="-1" aria-labelledby="editarUsuarioLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editarUsuarioLabel">Editar Rol</h5>
...
<script>
    $(document).on('click', '.view-permiso', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.roles.show", ":id") }}'.replace(':id', permisoId),
            method: 'GET',
    ...
    function eliminarPermiso(permisoId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.roles.destroy", ":id") }}'.replace(':id', permisoId),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
...
<script>
    function abrirModalEditar(userId) {
        ...
        $.ajax({
            url: '{{ route("admin.roles.edit", ":id") }}'.replace(':id', userId),
            type: 'GET',
            success: function(response) {
