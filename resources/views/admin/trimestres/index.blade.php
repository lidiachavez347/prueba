...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0">Lista de trimestres</h1>
        </div>
        <div class="col-sm-6">
        ...
        <tbody>
            ...
            <tr id="permiso-{{ $trimestre->id }}">
                ...
                <td>
                    ...
                    <!--- BOTON EDITAR--->
                    <a href="javascript:void(0);" onclick="abrirModalEditar('{{$trimestre->id}}')" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Editar">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Botón Eliminar Permiso -->
                    <button onclick="eliminarPermiso('{{ $trimestre->id }}')" class="btn btn-danger btn-sm" aria-label="eliminar" data-toggle="tooltip" data-placement="top" title="Eliminar">
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">Detalles del Trimestre</h5>
...
<script>
    ...
    $(document).on('click', '.view-permiso', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.trimestres.show", ":id") }}'.replace(':id', permisoId),
            method: 'GET',
            success: function(response) {
                $('#permisoDetails').html(response); // Carga los detalles en el modal
            },
            error: function(xhr) {
                Swal.fire('Error', 'No se pudo cargar los detalles del trimestre.', 'error');
            }
        });
    });

 ///elimnarrrrrrrrrrrrrrr
    function eliminarPermiso(permisoId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.trimestres.destroy", ":id") }}'.replace(':id', permisoId),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        if (response.success) {
                            ...
                            Toast.fire({
                                icon: 'success',
                                title: 'Trimestre eliminado exitosamente'
                            });
                    ...
                    error: function() {
                        Swal.fire('Error', 'No se pudo eliminar el trimestre', 'error');
                    }
                });
            }
        });
