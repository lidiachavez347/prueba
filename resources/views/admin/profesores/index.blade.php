...
<td>{{ $profesor->created_at }}</td>
                <td>
                    ...
                    <a href="javascript:void(0);" onclick="abrirModalEditar('{{$profesor->id}}')" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Editar">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Botón Eliminar Permiso -->
                    <button onclick="eliminarPermiso('{{ $profesor->id }}')" class="btn btn-danger btn-sm" aria-label="eliminar" data-toggle="tooltip" data-placement="top" title="Eliminar">
                    ...
                    <div class="modal fade" id="editarUsuarioModal" tabindex="-1" aria-labelledby="editarUsuarioLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editarUsuarioLabel">Editar Profesor</h5>
...
<script>
    $(document).on('click', '.view-permiso', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.profesores.show", ":id") }}'.replace(':id', permisoId),
            method: 'GET',
            success: function(response) {
                $('#permisoDetails').html(response);
            },
            error: function() {
                alert('Error al cargar los detalles del permiso.');
            }
        });
    });
    ...
    function eliminarPermiso(permisoId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.profesores.destroy", ":id") }}'.replace(':id', permisoId),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
...
<script>
    function abrirModalEditar(userId) {
        ...
        $.ajax({
            url: '{{ route("admin.profesores.edit", ":id") }}'.replace(':id', userId),
            type: 'GET',
            success: function(response) {
