...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0">Lista de temas</h1>
        </div>
        <div class="col-sm-6">
...
<div class="modal-footer">
    <a href="{{ route('profesor.temas.create') }}" class="btn btn-dark" data-toggle="tooltip" data-placement="left" title="Adicionar">
...
<div class="card body py-2 px-1">
    <table id="productos" class="table table striped shadow-lg mt-4table table-striped">
        ...
        <tbody>
            ...
            <tr id="permiso-{{ $tema->id }}">
                <td>{{ $tema->id }}</td>
                <td>{{ $tema->titulo }}</td>
                <td>{{ $tema->detalle }}</td>
                <td>{{ $tema->asignatura->nombre_asig }}</td>
                <td>{{ $tema->trimestre->periodo }}</td>
                <td width="70px" style="text-align: right">
                    @if ($tema->estado == 1)
                    <span class="badge badge-pill badge-success ">ACTIVO</span>
                    @elseif ($tema->estado == 0)
                ...
                <td>
                    ...
                    <a href="javascript:void(0);" onclick="abrirModalEditar('{{$tema->id}}')" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Editar">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Botón Eliminar Permiso -->
                    <button onclick="eliminarPermiso('{{ $tema->id }}')" class="btn btn-danger btn-sm" aria-label="eliminar" data-toggle="tooltip" data-placement="top" title="Eliminar">
                    ...
                    <div class="modal fade" id="editarUsuarioModal" tabindex="-1" aria-labelledby="editarUsuarioLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editarUsuarioLabel">Editar Tema</h5>
...
<script>
    $(document).on('click', '.view-permiso', function(e) {
        ...
        $.ajax({
            url: '{{ route("profesor.temas.show", ":id") }}'.replace(':id', permisoId),
            method: 'GET',
    ...
    function eliminarPermiso(permisoId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("profesor.temas.destroy", ":id") }}'.replace(':id', permisoId),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
...
<script>
    function abrirModalEditar(userId) {
        ...
        $.ajax({
            url: '{{ route("profesor.temas.edit", ":id") }}'.replace(':id', userId),
            type: 'GET',
            success: function(response) {
