...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0">Lista de Evaluaciones</h1>
        </div>
        <div class="col-sm-6">
...
<div class="modal-footer">
    ...
    <a href="{{ route('profesor.evaluaciones.create') }}" class="btn btn-dark" data-toggle="tooltip" data-placement="left" title="Adicionar">
        <i class="fa fa-plus-circle" aria-hidden="true"></i> Evaluaciones</a>
</div>
...
<div class="card body py-2 px-1">
    <table id="productos" class="table table striped shadow-lg mt-4table table-striped">
        ...
        <tbody>
            @foreach ($examenes as $tarea)
            <tr id="permiso-{{ $tarea->id }}">
                <td>{{ $tarea->id }}</td>
                <td>{{ $tarea->nombre }}</td>

                <td>{{ $tarea->tema->titulo }}</td>
                <td>{{ $tarea->criterio->nombres }}</td>
                <td>{{ $tarea->evaluar }}</td>
                <td>{{ $tarea->fecha }}</td>
                <td>{{ $tarea->trimestre->periodo }}</td>
                <td>{{ $tarea->asignatura->nombre_asig }}</td>
                <td>{{ $tarea->tipo }}</td>
                <td width="70px" style="text-align: right">
                    @if ($tarea->estado_eval == 1)
                    <span class="badge badge-pill badge-success ">ACTIVO</span>
                    @elseif ($tarea->estado_eval == 0)
                ...
                <td>
                    ...
                    @if($tarea->tipo == 'AUTOMATICO')
                    <a href="{{ route('profesor.evaluaciones.show',$tarea->id) }}"
                        class="btn btn-dark btn-sm"
                        title="Examen automatico">
                        <i class="fa fa-chalkboard-teacher" aria-hidden="true"></i> {{-- Icono para aula --}}
                    </a>
                    @elseif($tarea->tipo == 'ESCRITO')
                    <a href="{{ route('profesor.escrito.show',$tarea->id) }}"
                        class="btn btn-dark btn-sm"
                        title="Examen escrito">
                        <i class="fa fa-home" aria-hidden="true"></i> {{-- Icono para casa --}}
                    </a>
                    @endif
                    ...
                    <a href="javascript:void(0);" onclick="abrirModalEditar('{{$tarea->id}}')" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Editar">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Botón Eliminar Permiso -->
                    <button onclick="eliminarPermiso('{{ $tarea->id }}')" class="btn btn-danger btn-sm" aria-label="eliminar" data-toggle="tooltip" data-placement="top" title="Eliminar">
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">Detalles del Tarea</h5>
                    ...
                    <div class="modal fade" id="editarUsuarioModal" tabindex="-1" aria-labelledby="editarUsuarioLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editarUsuarioLabel">Editar Tarea</h5>
...
<script>
    $(document).on('click', '.view-permiso', function(e) {
        ...
        $.ajax({
            url: '{{ route("profesor.tareas.show", ":id") }}'.replace(':id', permisoId),
            method: 'GET',
            success: function(response) {
                $('#permisoDetails').html(response);
            },
            error: function() {
                alert('Error al cargar los detalles del trimestre.');
            }
        });
    });
    ...
    function eliminarPermiso(permisoId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("profesor.tareas.destroy", ":id") }}'.replace(':id', permisoId),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        if (response.success) {
                            ...
                            Toast.fire({
                                icon: 'success',
                                title: 'Tarea eliminado exitosamente'
                            });
...
<script>
    function abrirModalEditar(userId) {
        ...
        $.ajax({
            url: '{{ route("profesor.tareas.edit", ":id") }}'.replace(':id', userId),
            type: 'GET',
            success: function(response) {
